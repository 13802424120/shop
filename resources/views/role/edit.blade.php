        ...
        <form method="post" class="form-x" action="">
            ...
            <div class="form-group">
                ...
                <div class="field" style="padding-top:8px;">
                    @foreach ($permission_data as $v)
                        {{ str_repeat('-', 8 * $v['level']) }} <input type="checkbox" name="permission_id[]" level_id="{{ $v['level'] }}" value="{{ $v['id'] }}"
                        @if (in_array($v['id'], $role_permission_data))
                            checked="checked"
                        @endif
                        > {{ $v['permission_name'] }}
                        <br/>
